...
export const yesOrNo = async (
  ...
  const flag: boolean = /^\s*(y|n|yes|no)\s*$/i.test(answer)
 ...
 * @param fn
 */
export const doneWithClose =
  (fn: (...args: any[]) => Promise<void>): any =>
  (...args: any[]): void => {
    ;(async () => {
      await fn(...args)
      rl.close()
 ...
 * @param args
 */
export const collectOptionArgs = (arg: string, args: string[]): string[] => {
  args.push(arg)
  return args
}
