 ...
 */
export const relativePath = (directory: string, absoluteSourcePath: string, rootDirectory?: string) => {
  let relativePath: string
  if (_.isString(rootDirectory)) {
    const isSubDirectory = directory.startsWith(rootDirectory)
      && (directory === rootDirectory || /[/\\]/.test(directory.charAt(rootDirectory.length)))
    relativePath = isSubDirectory? path.relative(directory, absoluteSourcePath): absoluteSourcePath
  } else {
